...
namespace carl
{
    struct InternalCustomActionTypeOperations
    {
    ...
    template<typename T>
    std::shared_ptr<void> toOwningTypeErasedPtr(T data)
    {
    ...
    template<typename T>
    const T& fromOwningTypeErasedPtr(const std::shared_ptr<void>& ptr)
    {
    ...
    public:
        ...
        {
            using InternalT = InternalCustomActionTypeOperations::T;

            auto opsPtr = std::make_shared<decltype(operations)>(std::move(operations));

            InternalCustomActionTypeOperations internalOps{};
            ...
            internalOps.TryCreate = [opsPtr](const InputSample& current, const InputSample& prior) -> std::optional<InternalT> {
                ...
                {
                    return toOwningTypeErasedPtr(std::move(created.value()));
                }
                else
                {
                    return{};
            ...
            internalOps.Distance = [opsPtr](const InternalT& a, const InternalT& a0, const InternalT& b, const InternalT& b0, gsl::span<const NumberT> tuning) -> NumberT {
                const auto& aRef = fromOwningTypeErasedPtr<T>(a);
                const auto& a0Ref = fromOwningTypeErasedPtr<T>(a0);
                const auto& bRef = fromOwningTypeErasedPtr<T>(b);
                const auto& b0Ref = fromOwningTypeErasedPtr<T>(b0);
            ...
            internalOps.Lerp = [opsPtr](const InternalT& a, const InternalT& b, NumberT t) -> InternalT {
                const auto& aRef = fromOwningTypeErasedPtr<T>(a);
                const auto& bRef = fromOwningTypeErasedPtr<T>(b);
                return toOwningTypeErasedPtr(opsPtr->Lerp(aRef, bRef, t));
            };
    ...
    private:
        ...
        ContractId<>::IdT internalEnableCustomActionType(InternalCustomActionTypeOperations);
    };
}
